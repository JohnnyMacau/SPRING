...
function sendRequestForJobTitle(department_code, select_object) {
    ...
    for (var i = 0; i < len; i++) {
        if (department_code == job_list[i].key) {
            $("#" + select_object).html("<option value='-1' selected>请选择</option>");
...
function check(form, send_type) {
    ...
    /*if (document.getElementById('title1').value < 0 || document.getElementById('department_select1').value < 0) {
        ...
        complete = false;
        document.getElementById("error").innerHTML = "<font color='red'>红色部份为没填写或者格式错误</font><br>";
    }
  ...
  /*  if (document.getElementById("day").value < 0 || document.getElementById("month").value < 0 || document.getElementById("year").value < 0) {
        ...
        if (document.getElementById("year").value <= newYear) {
            if (document.getElementById("month").value < newMonth) {
                document.getElementById("day_font").color = 'red';
                complete = false;
                document.getElementById("c_error").innerHTML = "<font color='red'>不能为过去时间</font><br>";
  ...
  /*  if (document.getElementById("notice_day_type").value == "") {
    ...
    if (complete == true) {
        ...
        var url = "/operate" + "/profile/apply_for_a_job_on_behalf_of";
            ...
            "success": function (json) {
                ...
                if (json.stateCode === 200) {
                    if (send_type == 'onlySave') {
                        ...
                        // alert("申請成功")
                        // location = "/dispatch/profile/registered";
                        toastr.success('申請成功!')
                        setTimeout(function () {
                            location = "/dispatch/profile/registered";
                        }, 1500);
...
/*function getSource_id_select() {
    ...
    return String(myselect.options[myselect.selectedIndex].text).trim();
}*/
function getSource_id_select() {
    let myselect = document.getElementById("source_id");
...
function showExSourceInput(dbValue1, dbValue2, dbValue3) {
    ...
    if (v === "招聘网站" ||
        ...
        let html = '<font class="info_title" id="source_font">＊' + '该 ' + v + ' 是' +
            '</font>\n' +
        ...
        var url = "/ajax" + "/applicantSourceDetailList";
        $.ajax({
            "url": url,
            "type": "get",
            "async": false,
        ...
        $('#source_ref_div').append(
            '<font class="info_title" id="source_font">＊' + v + '出处' +
            '</font>\n' +
            '<input width="50px" type="text" id="source_ref"  name="sourceRef" value="' + dbValue1 +
            '"><br>')
    } else if (
        ...
        $('#source_ref_div').append(
            '<font class="info_title" id="source_font1">该员工的姓名</font>\n' +
            '<input type="text" name="sourceRef1" value="' + dbValue1 +
            '"><br>\n' +

            '<font class="info_title" id="source_font2">该员工的职位</font>\n' +
            '<input type="text" name="sourceRef2" value="' + dbValue2 +
            '"><br>\n' +

            '<font class="info_title" id="source_font3">该员工的部门</font>\n' +
...
$(function () {
    ...
    /* if ($.cookie("status") === "Y") {
         ...
         $.get("/applicant/pageNo", function (json) {
    ...
    $('#title1').on(
        {
            change: function () {
                ...
                if (jQuery_val > 0) {
                    ...
                    $(this).parent().append('<span style="font-size: 2px;color: red">此职位需轮班,请填写下方可轮班时间</span>')
...
let ARR_DAYOFWEEK = [
    ...
    '<option value="f1t5">周一至五</option>',
    '<option value="6 and 7">周六日</option>',
...
// alert(ARR_DAYOFWEEK.join())
    $("#add_part_Select_onShift").append(' <tr class="word" id="show1">\n' +
        '                                        <td height="30">&nbsp;</td>\n' +
        '                                        <td width="142">\n' +
        // '                                            <font class="info_title">＊请提供可轮班之时间<font></td>\n' +
        '\n' +
        '\n' +
        '                                        <td>\n' +
        ...
        '"\n' +
        '                                                    name="from_hour[]">\n' +
        '                                                <option value="-1" selected disabled style="display: none">时</option>\n' +
        ...
        '                                               style="cursor: pointer;width: 50px">－删除</div> </td>' +
        '                                    </tr>');
...
function setHistoryChooseJobList(num, val) {
    ...
    $.each(job_list, function (index, val0) {
        if (val0.deptPosDetailId == val.deptPosDetailId && val0.departmentId == val.departmentId) {
            count++;
    ...
    $1.append('<option  data-needshift="' + val.needShift +
        ...
        '</option>');
    // $1.parent().append('<span style="font-size: 2px;color: red">此职位需轮班,请填写下方可轮班时间</span>')
...
function getData1() {
    ...
    $.ajax({
        ...
        "success": function (json) {
            ...
            let info1 = json.data;
            if (info1.incompletePageNo <= 2) {
                return;
            }


            Object.keys(info1).map(function (key) {
            ...
            $.get("/applicant/t2/data", function (json) {
                if (json.stateCode !== 200) {
                    return;
                }
                let info2 = json.data;
            ...
            $.get("/applicant/t6/data", function (json) {
                if (json.stateCode !== 200) {
                    return;
                }
                let info3 = json.data;
...
function set_source_id_font() {
    var url = "/ajax" + "/msgDescSource?applicationOrg=FLT";
    $.ajax({
        "url": url,
        "type": "get",
        "async": false,
