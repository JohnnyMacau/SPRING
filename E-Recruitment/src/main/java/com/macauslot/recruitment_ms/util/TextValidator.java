...
public final class TextValidator {
	...
	private static final String REGEX_ID_Macao
			= "^[1|5|7][0-9]{6}\\(?[0-9A-Z]\\)?$";

	private static final String REGEX_ID_Hongkong
			= "^[A-Z]{1,2}[0-9]{6}\\(?[0-9A-Z]\\)?$";

	private static final String REGEX_ID_Passport
			= "/^1[45][0-9]{7}$|([P|p|S|s]\\d{7}$)|([S|s|G|g]\\d{8}$)|([Gg|Tt|Ss|Ll|Qq|Dd|Aa|Ff]\\d{8}$)|([H|h|M|m]\\d{8,10})$/";
//	private static final String REGEX_ID_Passport2
...
//		}
	...
	public static boolean checkID_Macao(String idCardNumber) {
		if ( StringUtils.isBlank(idCardNumber)) {
			return false;
		}
		return idCardNumber.matches(REGEX_ID_Macao);
	}
	public static boolean checkID_HongKong(String idCardNumber) {
		if ( StringUtils.isBlank(idCardNumber)) {
			return false;
		}
		return idCardNumber.matches(REGEX_ID_Hongkong);
	}
	public static boolean checkID_Passport(String idCardNumber) {
		if ( StringUtils.isBlank(idCardNumber)) {
			return false;
		}
		return idCardNumber.matches(REGEX_ID_Passport);
	}
